...
output_dir = os.path.join(current_dir, 'treesum_part2_results')
...
def run_ablation_treesum_2():
    os.makedirs(output_dir, exist_ok=True)
    dataset = load_dataset("Awesome075/multi_news_parquet", split="test")
    
    random.seed(random_seed)
    indices = random.sample(range(len(dataset)), num_samples)
    selected_indices = indices[500:1000]
    ...
    summary_df.to_csv(os.path.join(output_dir, 'treesum_part2_results.csv'), index=False)
    ...
    with open(os.path.join(output_dir, 'summaries_treesum_pt2_last_500.json'), 'w') as f:
        json.dump(results, f, indent=2)

if __name__ == "__main__":
    run_ablation_treesum_2()
