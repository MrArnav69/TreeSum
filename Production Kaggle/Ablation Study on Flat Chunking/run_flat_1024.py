"""
================================================================================
TREESUM ABLATION STUDY: FLAT 1024-TOKEN CHUNKING (NO OVERLAP)
================================================================================

Baseline 1 for the Chunking Strategy Ablation Study.

This script uses STRICT 1024-token chunking with:
- NO overlap
...
if os.path.exists('/kaggle/working'):
    OUTPUT_DIR = "/kaggle/working/results_flat_1024"
else:
    OUTPUT_DIR = "./results_flat_1024"
...
# ============================================================================
# FLAT CHUNKER (STRICT 1024 TOKENS, NO OVERLAP)
# ============================================================================
class FlatChunker1024:
    """
    Baseline Chunker: Strict 1024-token splits.
    
    This chunker:
    1. Cleans the document (whitespace normalization)
    2. Tokenizes the ENTIRE document
    3. Splits into chunks of EXACTLY 1024 tokens
    4. Decodes each chunk back to text
    
    NO overlap. NO sentence awareness. Pure token-based splitting.
    """
    ...
    def chunk_document(self, document: str) -> List[Dict]:
        """
        Split document into strict 1024-token chunks.
...
# ============================================================================
class HierarchicalSummarizerFlat:
    """
    Hierarchical Summarizer with Flat Chunking.
    
    The tree-reduction logic is IDENTICAL to the main TreeSum model.
    Only the chunker is replaced with FlatChunker1024.
    """
    ...
    def __init__(self, 
        ...
        # Initialize Flat Chunker (BASELINE)
        self.chunker = FlatChunker1024(self.tokenizer)
...
# ============================================================================
def run_ablation_flat_1024():
    """
    Run the Flat 1024 Chunking Ablation Study.
    """
    print("=" * 70)
    print("ABLATION STUDY: FLAT 1024-TOKEN CHUNKING (NO OVERLAP)")
    print("=" * 70)
    ...
    # Check if shared indices file exists (for consistency across all 3 ablation runs)
    ...
    print("\n[2/5] Initializing Hierarchical Summarizer (Flat Chunking)...")
    summarizer = HierarchicalSummarizerFlat()
    ...
    with open(os.path.join(OUTPUT_DIR, 'summaries_flat_1024.json'), 'w') as f:
        json.dump(results, f, indent=2)
    
    with open(os.path.join(OUTPUT_DIR, 'metrics_flat_1024.json'), 'w') as f:
        json.dump(metrics, f, indent=2)
    
    # 9. Print Summary
    print("\n" + "=" * 70)
    print("FINAL RESULTS: FLAT 1024-TOKEN CHUNKING")
    print("=" * 70)
