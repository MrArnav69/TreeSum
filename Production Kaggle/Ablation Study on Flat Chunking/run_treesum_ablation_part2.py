"""
================================================================================
TREESUM ABLATION STUDY: TREESUM (PART 2/2)
================================================================================

Total Integrated Script for Kaggle P100 GPU.
PART 2: Processing indices 500-1000
...
if os.path.exists('/kaggle/working'):
    OUTPUT_DIR = "/kaggle/working/results_treesum_ablation_part2"
else:
    OUTPUT_DIR = "./results_treesum_ablation_part2"
...
class SemanticDocumentChunker:
    ...
    def get_token_count(self, text: str, use_cache: bool = True) -> int:
        ...
        self._performance_stats['total_token_count_time'] += (time.time() - t_start)
    ...
    def get_sentence_embeddings(self, sentences: List[str], batch_size: int = 32) -> np.ndarray:
        ...
        self._performance_stats['total_embedding_time'] += (time.time() - t_start)
    ...
    def _find_optimal_split_point(self, sentences: List[str], current_tokens: List[int]) -> int:
        ...
        for i in range(len(smoothed_sims)):
            if cum_tokens[i] >= self.min_chunk_tokens and cum_tokens[i] <= self.max_tokens:
                valid_indices.append(i)
        
        if not valid_indices: 
            # Fallback: Ignore min_chunk_tokens, just respect max_tokens
            fallback_indices = [i for i in range(len(smoothed_sims)) if cum_tokens[i] <= self.max_tokens]
            if fallback_indices:
                return fallback_indices[-1] + 1
            return n_sentences
    ...
    def chunk_document(self, document: str):
        ...
        for idx, article in enumerate(articles):
            ...
            # Helper to finalize valid chunks
            def finalize_chunk(sentences, tokens, total):
                if not sentences: return
                coherence, metrics = self._compute_chunk_coherence(sentences)
                chunk = self._create_chunk_dict(len(all_chunks), sentences, total, idx, len(all_chunks)>0, 0, 0, 0, coherence, metrics)
            ...
            for para in paragraphs:
                ...
                # Strategy:
                # 1. If Paragraph fits in Remaining Buffer -> Add it.
                # 2. If Paragraph is Huge (> max_tokens) -> Must split it recursively (fallback to standard).
                # 3. If Paragraph overflows Buffer -> 
                #    a. Check if Buffer is "Full Enough" (e.g. > 80% or > min_tokens).
                #    b. If so, finalize Buffer (optimize split point). Start new Buffer with Overlap + Paragraph.
                #    c. If not, we MUST split the Paragraph to fill the Buffer (tight packing).
                
                if para_total > self.max_tokens:
                    # Case 2: Huge Paragraph.
                    # First, flush buffer if it has content
                    if current_buffer['sentences']:
                        finalize_chunk(current_buffer['sentences'], current_buffer['tokens'], current_buffer['total_tokens'])
                        # Prepare overlap for the huge paragraph processing
                        # (Ideally we'd seed the recursive splitter with overlap, but for simplicity here we just chunk the huge para)
                    
                    # Recursively chunk the huge paragraph
                    sub_chunks, _ = self.chunk_with_sentence_boundaries(para_sentences, idx, all_chunks[-1]['sentences'] if all_chunks else [], 0)
                    for c in sub_chunks:
                        c['chunk_id'] = len(all_chunks)
                        all_chunks.append(c)
                    current_buffer = {'sentences': [], 'tokens': [], 'total_tokens': 0} # Reset buffer after huge paragraph
                    continue
                ...
                if current_buffer['total_tokens'] + para_total > self.max_tokens:
                    ...
                    overlap = self.find_optimal_overlap_sentences(current_buffer['sentences'], self.overlap_tokens)
                    current_buffer = {
                        'sentences': overlap + para_sentences,
                        'tokens': [self.get_token_count(s) for s in overlap] + para_tokens,
                    ...
                    # Smart Balancing Check (SOTA 2.1: 80% Threshold)
                    if current_buffer['total_tokens'] > self.max_tokens * 0.8:
                         ...
                         if split_idx < len(current_buffer['sentences']):
                             ...
                             finalize_chunk(chunk_sents, chunk_toks, sum(chunk_toks))
                             
                             # Remaining
                             rem_sents = current_buffer['sentences'][split_idx:]
                             rem_toks = current_buffer['tokens'][split_idx:]
                             overlap = self.find_optimal_overlap_sentences(chunk_sents, self.overlap_tokens)
                             current_buffer = {
        ...
        self._performance_stats['total_chunking_time'] += (time.time() - start_time)
        self._performance_stats['num_documents_processed'] += 1
        return all_chunks
...
def run_ablation_treesum():
    print("=" * 70)
    print("ABLATION STUDY: TREESUM (PROPOSED METHOD) - PART 2/2")
    print("=" * 70)
    ...
    # SPLIT FOR PART 2: 500-1000
    indices = indices[500:]
    print(f"   PART 2: Processing last {len(indices)} indices (500-1000)")
    ...
    with open(os.path.join(OUTPUT_DIR, 'metrics_treesum_part2.json'), 'w') as f:
