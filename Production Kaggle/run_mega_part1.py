...
OUTPUT_DIR = '/kaggle/working/mega_ablation_part1'

# Create output directory
os.makedirs(OUTPUT_DIR, exist_ok=True)

logger.info(f"Configuration: {NUM_SAMPLES} samples, {len(ALPHA_VALUES)} alpha values, Device: {DEVICE}")
...
def prepare_data():
    """
    EXACT COPY from your original prepare_data function
    Load Multi-News dataset with deterministic sampling
    """
    print(f"Loading Multi-News dataset (Seed: {SEED})...")
    dataset = load_dataset("Awesome075/multi_news_parquet", split="test")
    
    # Deterministic selection of 1000 samples
    np.random.seed(SEED)
    all_indices = np.random.choice(len(dataset), TOTAL_SAMPLES, replace=False)
    
    # Partition for this run
    if PART == 1:
    ...
    logger.info(f"Selected {len(selected_samples)} samples (Part {PART}) from Multi-News dataset")
...
def run_alpha_sweep():
    ...
    for alpha in ALPHA_VALUES:
        ...
        # 5. Compute BERTScore
        print(f"\n  Computing BERTScore for Alpha={alpha:.1f}...")
        
        # Temporarily move Pegasus to CPU to free GPU memory for BERTScore
        if torch.cuda.is_available():
            print("  Moving Pegasus to CPU to free GPU memory...")
            summarizer.model.to('cpu')
            torch.cuda.empty_cache()
            print(f"  GPU Memory freed: {torch.cuda.memory_allocated()/1e9:.2f}GB allocated")
        
        # Compute BERTScore with smaller batch size to avoid OOM
        bert_scores = bertscore.compute(
            predictions=predictions, 
            references=references, 
            lang="en",
            model_type="microsoft/deberta-xlarge-mnli",  
            device="cpu", # Forced CPU for stability on large models
            batch_size=4  # Optimized for CPU memory
        )
        
        # Move Pegasus back to GPU for next alpha
        if torch.cuda.is_available() and alpha != ALPHA_VALUES[-1]:  # Don't move back on last iteration
            print("  Moving Pegasus back to GPU...")
        ...
        # 6. Save individual summaries for this alpha
        summary_export = []
...
if __name__ == "__main__":
    ...
    print("\n🎉 Experiment complete! Download results from /kaggle/working/alpha_sweep_results/")
