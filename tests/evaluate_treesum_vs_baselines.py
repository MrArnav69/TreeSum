#!/usr/bin/env python3
"""
Script to combine TreeSum summary files and evaluate against baseline models.
Combines 4 TreeSum files (p1-p4) to get 2000 samples and evaluates ROUGE scores
...
def extract_matching_samples(baseline_data: List[Dict[str, Any]], 
    ...
    # Create mapping from sample_id to treesum data (first 2000 samples)
...
def calculate_bert_scores(references: List[str], candidates: List[str], model_name: str = "microsoft/deberta-xlarge-mnli") -> Dict[str, float]:
    """Calculate BERTScore using specified model."""
    print(f"Calculating BERTScore with {model_name}...")
    
    P, R, F1 = bert_score(candidates, references, lang="en", model_type=model_name, verbose=True)
...
def main():
    ...
    print("=== TreeSum vs Baselines Evaluation ===\n")
    ...
    for model_name, baseline_samples in baseline_data.items():
        ...
        # Calculate BERTScore
        baseline_bert_scores = calculate_bert_scores(references, baseline_candidates)
        treesum_bert_scores = calculate_bert_scores(references, treesum_candidates)
        
        # Combine scores
        baseline_scores.update(baseline_bert_scores)
        treesum_scores.update(treesum_bert_scores)
        ...
        print(f"  Baseline BERT-P:  {baseline_scores['bert_precision']:.4f}")
        print(f"  TreeSum BERT-P:   {treesum_scores['bert_precision']:.4f}")
        print(f"  Baseline BERT-R:  {baseline_scores['bert_recall']:.4f}")
        print(f"  TreeSum BERT-R:   {treesum_scores['bert_recall']:.4f}")
        print(f"  Baseline BERT-F1: {baseline_scores['bert_f1']:.4f}")
        print(f"  TreeSum BERT-F1:  {treesum_scores['bert_f1']:.4f}")
    
    # Save results to file
    output_file = "/Users/mrarnav69/Documents/TreeSum/tests/evaluation_results.json"
    ...
    for model_name, model_results in results.items():
        ...
        for metric in ['bert_precision', 'bert_recall', 'bert_f1']:
            ...
            print(f"{model_name:<10} {metric_name:<8} {baseline_score:<10.4f} {treesum_score:<10.4f} {diff:+.4f}")
